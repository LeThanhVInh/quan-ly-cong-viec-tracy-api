...
namespace API_PhanCongCongViec.Controllers
{
    ...
    public class ProjectMemberController : Controller
    {
        public object GetByProjectID(int projectID)
        {
            ...
            {
                DataTable item = Connect.GetTable(@"select * from tb_PROJECT_MEMBER where projectID=@projectID",
        ...
        public object Delete(int userID, int projectID)
        {
            ...
            if (AuthenFunctionProviders.CheckValidate(Request.Headers))
            {
                if (Connect.Exec(@"delete from tb_PROJECT_MEMBER where userID=@userID and projectID=@projectID",
        ...
        public object insert([FromBody] dynamic item)
        {
            ...
            if (AuthenFunctionProviders.CheckValidate(Request.Headers))
            {
                try
                {
                    ...
                    {
                        if (Connect.Exec(@"INSERT INTO tb_PROJECT_MEMBER(userID,projectID)
